...
void AsyncRgbLedAnalyzer::WorkerThread()
{
    ...
    int channelsTotal = mSettings->LEDChannelCount();
    ...
    for( ;; )
    {
        ...
        for( ;; )
        {
            ...
            if( result.mValid )
            {
                ...
                else if( channelsTotal == 4 )
                {
                    frame.mData1 = result.mRGBW.ConvertToU64();
                    frame.mData2 = frameInPacketIndex++;
                    mResults->AddFrame( frame );

              
                    frame_v2.AddInteger( "index", frame.mData2 );
                    frame_v2.AddInteger( "red", result.mRGBW.red );
                    frame_v2.AddInteger( "green", result.mRGBW.green );
                    frame_v2.AddInteger( "blue", result.mRGBW.blue );
                    frame_v2.AddInteger( "white", result.mRGBW.white );
                    mResults->AddFrameV2( frame_v2, "rgbwpixel", frame.mStartingSampleInclusive, frame.mEndingSampleInclusive );
                }
...
auto AsyncRgbLedAnalyzer::ReadRGBTriple() -> RGBResult
{
    ...
    if( channel == channelsTotal )
    {
        // we saw three or four complete channels, we can use this
        if( channelsTotal == 3 )result.mRGB = RGBValue::CreateFromControllerOrder( mSettings->GetColorLayout(), channels );
        else if( channelsTotal == 4 )  result.mRGBW = RGBWValue::CreateFromControllerOrder( mSettings->GetColorLayout(), channels );
        result.mValid = true;
...
{
    return "Addressable LEDs (Custom1)";
}

const char* GetAnalyzerName()
{
    return "Addressable LEDs (NLED-Rev1)";
}
